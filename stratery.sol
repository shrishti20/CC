...
contract StratSimple is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public entranceFeeFactor = 100; // 1% entrance fee. set in BPS
    uint256 public constant entranceFeeFactorMax = 1000; // 0.5% is the max entrance fee settable. set in BPS
     uint256 public exitFeeFactor = 100;
    uint256 public constant exitFeeFactorMax = 1000;
    constructor(
        address _rewardAddress,
        address _CAPLAddress,
        ...
        CAPLAddress = _CAPLAddress;
        feeReceiver = _feeReceiver;
        wantAddress = _wantAddress;
        transferOwnership(rewardAddress);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 feeAmount = _wantAmt.mul(entranceFeeFactor).div(ONE_IN_BPS);
        IERC20(wantAddress).safeTransfer(feeReceiver, feeAmount);
        uint256 sharesAdded = _wantAmt.sub(feeAmount);
        ...
        returns (uint256)
    {
        ...
        uint256 feeAmount = _wantAmt.mul(exitFeeFactor).div(ONE_IN_BPS);
        IERC20(wantAddress).safeTransfer(feeReceiver, feeAmount);
        uint256 amount = _wantAmt.sub(feeAmount);
        sharesTotal = sharesTotal.sub(sharesRemoved);
        wantLockedTotal = wantLockedTotal.sub(_wantAmt);

        IERC20(wantAddress).safeTransfer(rewardAddress, amount);
    ...
    function setEntranceFeeFactor(uint256 _entranceFeeFactor) public onlyGovernance {
     ...
     function setExitFeeFactor(uint256 _exitFeeFactor) public onlyGovernance {
      require(_exitFeeFactor <= entranceFeeFactorMax, "!safe - too high");
    ...
    function setGov(address _govAddress) public onlyGovernance {
     ...
     function setFeeReceiver(address _feeAddress) public onlyGovernance {
