...
contract Swap is Ownable {
    ...
    bool public swapAndLiquifyEnabled = false; // Disable by default
     ...
     modifier lockTheSwap {
      ...
      constructor(address _capl, address _usdc) public {
    capl = (_capl);
    usdc = (_usdc);
       ...
       IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
            ...
            caplUsdcPair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(capl, usdc);
      ...
      function getUSDC(uint256 tokenAmount, uint256 duration) external{
        IERC20(capl).transferFrom(msg.sender, address(this),tokenAmount) ; 
        swapCaplForUSDC(tokenAmount, msg.sender, duration);
        
      }
      
       function getCapl(uint256 tokenAmount, uint256 duration) external{
        IERC20(usdc).transferFrom(msg.sender, address(this),tokenAmount) ; 
        swapUsdcForCapl(tokenAmount, msg.sender, duration);
     ...
     function setSlippageTolerance(uint256 slippage) external onlyOwner{
    ...
    function getUSDCAmount(uint256 tokenAmount) external view returns(uint256){
     ...
     function getCaplAmount(uint256 tokenAmount) external view returns(uint256){
      ...
      function swapCaplForUSDC(uint256 tokenAmount, address sender, uint256 duration) private{
      ...
      function swapUsdcForCapl(uint256 tokenAmount, address sender, uint256 duration)  private{
